...
with tab_chat:
    ...
    # ── 图片附加栏（在 chat_input 上方）──────────────────
    attach_col, upload_col = st.columns([1, 3])
    with attach_col:
        paste_result = pbutton(
            "📋 粘贴图片",
            background_color="#f0f7ff",
            hover_background_color="#ddf4ff",
            key="paste_btn"
        )
        if paste_result.image_data is not None:
            ...
            st.session_state.pasted_images.append(f"data:image/png;base64,{b64}")
            st.rerun()
    ...
    # 合并粘贴图 + 上传图
    all_images = list(st.session_state.pasted_images)
    if uploaded_files:
    ...
    # 显示待发送图片缩略图
    if all_images:
        thumb_cols = st.columns(min(len(all_images), 6))
        for i, uri in enumerate(all_images):
            with thumb_cols[i % 6]:
                st.image(uri_to_bytes(uri), use_container_width=True)
        if st.button("✕ 清除图片", key="clear_imgs"):
            st.session_state.pasted_images = []
            st.rerun()
        st.caption(f"共 {len(all_images)} 张图片，发送问题后一并分析")

    # ── 聊天输入 ──────────────────────────────────────────
    new_msg_slot = st.container()
    user_input   = st.chat_input("输入问题，例如：这条路大门怎么制作？")
    st.divider()
    ...
    if user_input:
        ...
        st.session_state.messages.append({
            "role": "user", "content": user_input, "images": images
        })
        with new_msg_slot:
            ...
            with new_msg_slot:
                with st.chat_message("assistant"):
                        ...
                        resp1 = client.chat.completions.create(
                            ...
                            messages=[
                                ...
                                {"role": "user", "content": image_blocks + [{
                                    "type": "text",
                                    "text": "分析图片场景。" + (f"用户说明：{user_input}" if user_input else "")
                                }]}
                            ]
                        )
                    ...
                    with st.status("检索相关工艺规范…", expanded=False) as s2:
                        search_q = " ".join(keywords) + " " + user_input
